    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                current_app.logger.info(f"Access token verification failed: {str(access_token_error)}")
                
                try:
                    # 액세스 토큰이 실패하면 리프레시 토큰으로 갱신 시도
                    ...
                    user = User.find_by_user_id(user_id)
                    if not user:
                        current_app.logger.warning(f"User not found during token refresh: {user_id}")
                    ...
                    current_app.logger.warning(f"Refresh token verification failed: {str(refresh_token_error)}")
                    # 리프레시 토큰도 실패하면 로그인 페이지로 리다이렉트
                    return redirect(url_for('main.login'))
